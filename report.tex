...
March. By the end of the sprint, the entirety of the sprint and product 
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width = \linewidth]{PI DB User class}
  \caption{User Class in our database model}
  \label{fig:User_Table}
\end{figure}

Figure \ref{fig:User_Table} describes our  \texttt{User} Table: 
Used to be central to our system, the User table captures
...
for ensuring secure user authentication and system access. This table
acts as the primary entity with which other tables associate,
...
in subordinate tables, reflecting the diverse activities and metrics
...
capabilities, should they be developed in the future. This table covers ARQ 1.

\subsection{UML Class diagram}

We use the Dapper module on \texttt{C#} since it allows us to return or insert a list of \texttt{C#} objects from an sql query instead of retrieving them as tuples. Hence, based on the ERD, all the classes are built for the Dapper package./par
...
have completed that day. Adjacent to this bar, a plus button is the way to confirm the work
users inserted and start to time it, allowing users to easily update their daily work achievements.\par
...
\hline
 & \textbf{Week 1} & \textbf{Week 2} & \textbf{Week 3} & \textbf{Week 4} & \textbf{Week 5} \\
\hline
